...
function GameObject(x, y, vertices, radialPoints, tag, update, lateUpdate, systemUpdate, collideFunction, animations, currAnimation, defaultFrame, defaultLength, defaultWidth
					, debugMode, properties, canvas, context, audio, init, postInit, nilCollideFunction, xRenderFactor, yRenderFactor, skeletalJoints, spriteSheet, skeletalSheetMappings, defaultCanvasLayer) {
	this.position = {x, y};
	...
	this.defaultCanvasLayer = defaultCanvasLayer;
...
function renderAnimationObject(gameObj) {
	...
	for(var i = 0; i < gameObj.skeletalJoints.length; i++) {
		...
		if(gameObj.skeletalSheetMappings[i].nodeImg.w != null) {
			...
			var contextUse = gameObj.handler.LocalCanvases[gameObj.skeletalSheetMappings[i].nodeImg.cLayer].context;
		...
		for(var j = 0; j < gameObj.skeletalJoints[i].joints.length; j++) {
			...
			if(gameObj.skeletalSheetMappings[i] != null) {
				if(gameObj.skeletalSheetMappings[i].jointImgs[gameObj.skeletalJoints[i].joints[j]] != null) {
					...
					var contextUse = gameObj.handler.LocalCanvases[gameObj.skeletalSheetMappings[i].jointImgs[gameObj.skeletalJoints[i].joints[j]].cLayer].context;
...
function CreateLocalCanvasesDev(BackgroundColor) {
	var newCanvasID = 'Canvas' + oHandlerDev.LocalCanvases.length;
	
	var canvasSection = document.getElementById('CanvasesDev');
	...
	if(oHandlerDev.LocalCanvases.length == 0) {
		canv0.style.marginTop = '0px';
	} else {
	...
	if(BackgroundColor != null) {
		canv0.style.background = BackgroundColor;
	}
	
	canv0.style.zIndex = oHandlerDev.LocalCanvases.length + 5;
	...
	oHandlerDev.LocalCanvases.push({canvas: newCanv0, context: newCtx0});
}

function CreateLocalCanvases(BackgroundColor) {
	var newCanvasID = 'Canvas' + oHandler.LocalCanvases.length;
	...
	canv0.height = 576;
	canv0.style.display = 'block';
	if(oHandler.LocalCanvases.length == 0) {
		canv0.style.marginTop = '0px';
	} else {
		canv0.style.marginTop = '-576px';
	}
	...
	canv0.style.zIndex = oHandler.LocalCanvases.length + 5;

	canvasSection.appendChild(canv0); // adds the canvas to #someBox
	
	var newCanv0 = document.getElementById(newCanvasID);
	var newCtx0 = newCanv0.getContext("2d");
	oHandler.LocalCanvases.push({canvas: newCanv0, context: newCtx0});
}
...
function storeClick_DefaultDev(e) {
	...
	oHandlerDev.cBuff.push(mousepos);
}


function addClickToCanvasDev(canIndex) {
	//DefaultDevGlobals.currHandler = this.handler;
	console.log(LocalCanvases);
	console.log(canIndex);
	oHandlerDev.LocalCanvases[canIndex].canvas.addEventListener('click', storeClick_DefaultDev, false);
}

function storeClick_Default(e) {
	console.log(this);
	var mousepos = getMousePos(this,e);
	oHandler.cBuff.push(mousepos);
}
...
function addClickToCanvas(canIndex) {
	...
	console.log(LocalCanvases);
	console.log(canIndex);
	oHandler.LocalCanvases[canIndex].canvas.addEventListener('click', storeClick_Default, false);
}
...
function ObjectHandler() {
	...
	this.renderObjects = function(PlayArea,PlayCanvas, BackgroundArea, BackgroundCanvas, ForegroundArea, ForegroundCanvas, UIArea, UICanvas) {
		...
		for(var canI = 0; canI < this.LocalCanvases.length; canI++) {
			this.LocalCanvases[canI].context.clearRect(0, 0, this.LocalCanvases[canI].canvas.width, this.LocalCanvases[canI].canvas.height);
		}
		...
		for(var objI = 0; objI < this.Objects.length; objI++) {
			...
			var canvasesReady = true;
			if(this.LocalCanvases.length == 0) {
				canvasesReady = false;
			}
			
			if(canvasesReady) {
				var contextUse = this.LocalCanvases[this.Objects[objI].defaultCanvasLayer].context;
					...
					if(this.Objects[objI].animations[0].texture) {
						...
						for(var i = 0; i <= MaxI; i++) {
							for(var j = 0; j <= MaxJ; j++) {
								...
								contextUse.drawImage(img, 0, 0, widthDraw, heightDraw, xPos + drawOffsetX, yPos + drawOffsetY, widthDraw, heightDraw);
							}
						}
					}
				}
				...
				if(!textured && !skeletal) {
					...
					contextUse.drawImage(img, xPos + drawOffsetX, yPos + drawOffsetY, cWidth, cLength);
				}
				...
				*/
				...
				if(this.Objects[objI].debugMode) {
					var contextUsePlay = this.LocalCanvases[this.Objects[objI].defaultCanvasLayer].context;
					...
					for(var verI = 0; verI < this.Objects[objI].vertices.length; verI++) {
						...
						contextUsePlay.beginPath();
						contextUsePlay.moveTo(startX, startY);
						...
						contextUsePlay.lineTo(endX, endY);
						
						contextUsePlay.stroke();
					}
					...
					for(var radI = 0; radI < this.Objects[objI].radialPoints.length; radI++) {
						...
						contextUsePlay.beginPath();
						contextUsePlay.arc(radX, radY, cRadius, 0, 2 * Math.PI, false);
						contextUsePlay.stroke();
					}
...
function createGOFromJSON(jArg,DevFlag) {
	...
	if(jArg.defaultCanvasLayer != null) {
		defaultCanvasLayer = jArg.defaultCanvasLayer;
	}
	
	var newGameObject = new GameObject(x,y,vertices,radialPoints,tag,update,lateUpdate,systemUpdate,collideFunction,animations,currAnimation,defaultFrame,defaultLength,defaultWidth,debugMode,properties,canvas,context,audio,init,postInit,nilCollideFunction,xRenderFactor,yRenderFactor,skeletalJoints,spriteSheet,skeletalSheetMappings,defaultCanvasLayer);
...
function createGOJsonFromString(args) {
	...
	for(var i = 0; i < argsList.length; i++) {
		...
		else if(argsList[i] == '-dcl') {
			if(!isNaN(argsList[i + 1]) && argsList[i + 1] != null && argsList[i + 1] != '') {
				jsonArg.defaultCanvasLayer = parseFloat(argsList[i + 1]);
				i++;
			}
		}
	}
